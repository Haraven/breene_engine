...
out vec3 w_pos_tese[3];
out PNPatch patch_tese[3];

float wij(int i, int j)
{
    return dot(w_pos_tesc[j].xyz - w_pos_tesc[i].xyz, normal_tesc[i]);
}

float vij(int i, int j)
{
    vec3 pdiff = w_pos_tesc[j].xyz - w_pos_tesc[i].xyz;
...
void main()
{
    ...
    float p0 = w_pos_tesc[0][gl_InvocationID];
    float p1 = w_pos_tesc[1][gl_InvocationID];
    float p2 = w_pos_tesc[2][gl_InvocationID];
